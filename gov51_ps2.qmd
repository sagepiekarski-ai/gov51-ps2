...
q3.5_stats <- q3.3 %>% 
  group_by(treatment) %>% 
  summarize(variance = var(therm2, na.rm = TRUE),
            n = sum(!is.na(therm2)))
q3.5_stats

#create subsets
control_vals <- q3.3$therm2[q3.3$treatment == "No Contact" & !is.na(q3.3$therm2)]
treated_vals <- q3.3$therm2[q3.3$treatment == "Same-Sex Marriage Script by Gay Canvasser" & !is.na(q3.3$therm2)]

# variance and n
var_control <- var(control_vals)
n_control <- length(control_vals)

var_treated <- var(treated_vals)
n_treated <- length(treated_vals)

#calculate the standard error
stand_e <- sqrt(var_control/n_control + var_treated/n_treated)
stand_e

#diff in means
diff_means <- mean(treated_vals) - mean(control_vals)
diff_means

#construct confidence interval
ci_low <- diff_means - 1.96*stand_e
ci_up <- diff_means + 1.96*stand_e
...
study2_correlation2 <- cor(q3.6$therm1, q3.6$therm2, use = "complete.obs")
study2_correlation2
study2_correlation3 <- cor(q3.6$therm1, q3.6$therm3, use = "complete.obs")
study2_correlation3
study2_correlation4 <- cor(q3.6$therm1, q3.6$therm4, use = "complete.obs")
study2_correlation4
